...
class Generator:
    ...
    def init_map(self):
        self.map_size_width = random.randint(4, 10)
    ...
    def valid_position_checker_for_init_shelves(self, position_x, position_y, direction): #positionがはみ出ていないか or not_freeでないかを確認する
    ...
    def init_shelves(self):
        ...
        self.number_of_shelves = random.randint(1, self.number_of_accesible_positions if self.number_of_accesible_positions <= 10 else 10)
        ...
        for i in range(self.number_of_shelves): #棚の初期化                        
            count = 0
            threshold = 10000             
            position_x = 0
            position_y = 0
            ...
            while(self.valid_position_checker_for_init_shelves(position_x, position_y, direction) == 0): #どこにも置けない場合無限ループになってしまうので閾値でループ回数を制限する
                if(count > threshold):
                    break                
            ...
            self.list_of_shelves.append({'name': str(i), 'position': (position_x, position_y), 'accesible_position': accesible_position, 'direction': direction})

        #print(self.list_of_shelves)

    def init_orders(self):
        self.number_of_orders = random.randint(1, len(self.list_of_shelves))
        self.list_of_orders = []
        
        for _ in range(self.number_of_orders):
            number_of_order = random.randint(1, len(self.list_of_shelves))
            list_of_order = random.sample(self.list_of_shelves, number_of_order)
    ...
    def valid_map_checker(self): #bfsを用いてマップの有効性(全ての商品に対して、それらを獲得できるようなパスが必ず一つ存在する)を確認する
        ...
        count = 0
        visited = []
        valid_condition = (self.map_size_width * (self.map_size_height - 1)) - self.number_of_shelves
